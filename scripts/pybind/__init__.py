...
from .functions import Constructor, DisabledConstructor, MemberFunction, StaticFunction
...
class Class(FunctionContainer):
    def __init__(self, name, *argv, cpp_name=None, disable_constructor=False, **kwargs):
    ...
    def GenerateRegister(self, w):
        w.Write(f"if (PyType_Ready(&{self.type_object_name()}) != 0) return nullptr;")
        w.Write(
            f'PyModule_AddObject(module, "{self.name}", '
