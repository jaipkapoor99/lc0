...
from .retval import NoneRetVal, RetVal
...
class Function(PyObject):
    def __init__(self, name, gen_function_name, self_type=None, param_type=None):
        super().__init__(name)
        self.parameters = []
        self.exceptions = []
    ...
    def _generate_parse_params(self, w):
        if self.param_type() == FunctionType.METH_KEYWORDS:
            ...
            w.Write(
                "const char* keywords[] = {%s};"
                % ", ".join([f'"{x.name}"' for x in self.parameters] + ["nullptr"])
            )
            ...
            params_list = ",\n    ".join(
                ...
                + [x for xs in self.parameters for x in xs.parse_tuple_sink_list()]
            )
        ...
        raise NotImplementedError(
            "Not implemented function type %d" % self.param_type()
        )
...
class MemberFunction(Function):
    ...
    def _generate_call(self, w):
        if isinstance(self.retval, NoneRetVal):
            w.Write(f"self->value->{self.cpp_name}({self._list_caller_params()});")
        else:
            w.Write(
                f"{self.retval.cpp_type()} "
...
class StaticFunction(Function):
    ...
    def _generate_call(self, w):
        if isinstance(self.retval, NoneRetVal):
            w.Write(
                f"{self.cpp_type_name}::{self.cpp_name}({self._list_caller_params()});"
            )
        else:
            w.Write(
                f"{self.retval.cpp_type()} "
...
class Constructor(Function):
    ...
    def _generate_call(self, w):
        w.Write(
            f"self->value = new {self.cpp_type_name}({self._list_caller_params()});"
        )
